...
import { Check, Clipboard, Copy, Download, FilePlus2, FolderOpen, Maximize2, Redo2, Save, ShieldCheck, Sparkles, Undo2, MousePointer2, Hand, Route, Square, Triangle, Type, Trash2, Bot, Code2, DollarSign, Image, FileText } from 'lucide-react';
...
function Designer(){
 ...
 const bringForward=()=>{const ids=new Set(selectedIds());if(!ids.size)return;pushHistory();setNodes(c=>[...c.filter(n=>!ids.has(n.id)),...c.filter(n=>ids.has(n.id))]);markChanged();setContextMenu(null);};
 const sendBackward=()=>{const ids=new Set(selectedIds());if(!ids.size)return;pushHistory();setNodes(c=>[...c.filter(n=>ids.has(n.id)),...c.filter(n=>!ids.has(n.id))]);markChanged();setContextMenu(null);};
 ...
 useEffect(()=>{const down=(e:KeyboardEvent)=>{if(e.code==='Space'&&!['INPUT','TEXTAREA','SELECT'].includes((e.target as HTMLElement).tagName)){e.preventDefault();setTool('hand');}};const up=(e:KeyboardEvent)=>{if(e.code==='Space')setTool('select');};window.addEventListener('keydown',down);window.addEventListener('keyup',up);return()=>{window.removeEventListener('keydown',down);window.removeEventListener('keyup',up);};},[]);
