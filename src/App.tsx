...
const selectedNode=nodes.find(n=>n.id===selectedNodeId);const selectedEdge=edges.find(e=>e.id===selectedEdgeId);const isArchitecture=selectedNode&&selectedNode.type!=='drawing';const isDrawing=selectedNode?.type==='drawing';
 ...
 const saveDesign=useCallback(()=>{
  const document={version:'7.5.0',designId,designName,organizationId,organizationName,projectId,projectName,environmentId,environmentName,nodes,edges,variables:designVariables,locals:designLocals,updatedAt:new Date().toISOString()};
  ...
  setSaveState('saved');
},[designId,designName,organizationId,organizationName,projectId,projectName,environmentId,environmentName,nodes,edges,designVariables,designLocals]);const loadDesign=()=>{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return alert('No saved design found.');const p=JSON.parse(raw);setDesignId(p.designId||crypto.randomUUID());setOrganizationId(p.organizationId||organizationId);setOrganizationName(p.organizationName||organizationName);setProjectId(p.projectId||projectId);setProjectName(p.projectName||projectName);setEnvironmentId(p.environmentId||environmentId);setEnvironmentName(p.environmentName||environmentName);setNodes(recalcHierarchy(p.nodes||[]));setEdges(p.edges||[]);setDesignName(p.designName||'Loaded Architecture');setDesignVariables(p.variables||[]);setDesignLocals(p.locals||[]);setSaveState('saved');setTimeout(()=>fitView({padding:.18}),0);};const download=(blob:Blob,name:string)=>{const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;a.click();URL.revokeObjectURL(u);};const safeFileName=()=>designName.replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'').toLowerCase()||'archmindcanvas-diagram';
 ...
 return <div className="app-shell"><header className="topbar"><div className="brand-block"><div className="brand-mark"><Sparkles size={19}/></div><div><div className="brand">ArchMindCanvas</div><div className="subtitle">AI Architecture Studio · Describe. Design. Validate. Generate IaC.<small className="founder-line">Founded by Pranab Baro</small></div></div></div><button className="enterprise-breadcrumb" onClick={()=>setScopeManagerOpen(true)} title="Organization / Project / Environment / Architecture">
