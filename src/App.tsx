...
import type { LocalDefinition, VariableDefinition, ArchitectureNode, CanvasEdge, CanvasNode, ConnectorStyle, DrawingNode, ResourceType, ValidationFinding, ArchitectureNodeData, TagMap } from './types';
...
function Designer(){
 ...
 useEffect(()=>{
  const raw=localStorage.getItem('archmindcanvas-global-variables');
  if(raw){try{const p=JSON.parse(raw);setDeclaredVariables(p.variables||[]);setDeclaredLocals(p.locals||[]);}catch{}}
},[]);
useEffect(()=>{
  localStorage.setItem('archmindcanvas-global-variables',JSON.stringify({variables:declaredVariables,locals:declaredLocals}));
},[declaredVariables,declaredLocals]);
const selectedNode=nodes.find(n=>n.id===selectedNodeId);const selectedEdge=edges.find(e=>e.id===selectedEdgeId);const isArchitecture=selectedNode&&selectedNode.type!=='drawing';const isDrawing=selectedNode?.type==='drawing';
 ...
 const importIacToDiagram=()=>{
  ...
  if(iacImportType==='json'){
    try{
      ...
      setDesignId(parsed.designId||crypto.randomUUID());setDesignName(parsed.designName||'Imported Architecture');setDesignVariables(parsed.variables||[]);setDesignLocals(parsed.locals||[]);
 ...
 const saveDesign=useCallback(()=>{
  const document={version:'7.3.8',designId,designName,nodes,edges,variables:designVariables,locals:designLocals,updatedAt:new Date().toISOString()};
  ...
  setSaveState('saved');
},[designId,designName,nodes,edges,designVariables,designLocals]);const loadDesign=()=>{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return alert('No saved design found.');const p=JSON.parse(raw);setDesignId(p.designId||crypto.randomUUID());setNodes(recalcHierarchy(p.nodes||[]));setEdges(p.edges||[]);setDesignName(p.designName||'Loaded Architecture');setDesignVariables(p.variables||[]);setDesignLocals(p.locals||[]);setSaveState('saved');setTimeout(()=>fitView({padding:.18}),0);};const download=(blob:Blob,name:string)=>{const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;a.click();URL.revokeObjectURL(u);};const safeFileName=()=>designName.replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'').toLowerCase()||'archmindcanvas-diagram';
 ...
 {designVariablesOpen&&<div className="design-vars-overlay" onMouseDown={()=>setDesignVariablesOpen(false)}>
  <div className="design-vars-dialog" onMouseDown={e=>e.stopPropagation()}>
    <div className="design-vars-dialog-head">
      <div>
        <strong>Global Variables & Locals</strong>
        <small>Available to all designs in this browser workspace</small>
      </div>
      <button onClick={()=>setDesignVariablesOpen(false)} aria-label="Close">×</button>
    </div>
    <div className="design-vars-scope">
      <Variable size={14}/>
      <span>These declarations are global and may be referenced by resources in any design.</span>
    </div>
