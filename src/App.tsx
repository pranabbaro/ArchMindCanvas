...
function Designer(){
 ...
 const importIacToDiagram=()=>{
  ...
  if(iacImportType==='json'){
    try{
      ...
      setDesignId(parsed.designId||crypto.randomUUID());setDesignName(parsed.designName||'Imported Architecture');setDeclaredVariables(parsed.variables||[]);setDeclaredLocals(parsed.locals||[]);
 ...
 const saveDesign=useCallback(()=>{
  const document={version:'7.3.8',designId,designName,nodes,edges,variables:declaredVariables,locals:declaredLocals,updatedAt:new Date().toISOString()};
  ...
  setSaveState('saved');
},[designId,designName,nodes,edges,declaredVariables,declaredLocals]);const loadDesign=()=>{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return alert('No saved design found.');const p=JSON.parse(raw);setDesignId(p.designId||crypto.randomUUID());setNodes(recalcHierarchy(p.nodes||[]));setEdges(p.edges||[]);setDesignName(p.designName||'Loaded Architecture');setDeclaredVariables(p.variables||[]);setDeclaredLocals(p.locals||[]);setSaveState('saved');setTimeout(()=>fitView({padding:.18}),0);};const download=(blob:Blob,name:string)=>{const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;a.click();URL.revokeObjectURL(u);};const safeFileName=()=>designName.replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'').toLowerCase()||'archmindcanvas-diagram';
 ...
 {designVariablesOpen&&<div className="design-vars-overlay" onMouseDown={()=>setDesignVariablesOpen(false)}>
  <div className="design-vars-dialog" onMouseDown={e=>e.stopPropagation()}>
    <div className="design-vars-dialog-head">
      <div>
        <strong>Design Variables & Locals</strong>
        <small>{designName} · Scoped only to this design</small>
      </div>
      <button onClick={()=>setDesignVariablesOpen(false)} aria-label="Close">×</button>
    </div>
    <div className="design-vars-scope">
      <Variable size={14}/>
      <span>These declarations are shared by every resource in this design and saved with the design document.</span>
    </div>
