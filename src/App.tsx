...
function Designer(){
 ...
 const importIacToDiagram=()=>{
  ...
  if(iacImportType==='json'){
    try{
      ...
      setDesignName(parsed.designName||'Imported Architecture');setDeclaredVariables(parsed.variables||[]);setDeclaredLocals(parsed.locals||[]);
 ...
 const saveDesign=useCallback(()=>{localStorage.setItem(STORAGE_KEY,JSON.stringify({designName,nodes,edges,variables:declaredVariables,locals:declaredLocals}));setSaveState('saved');},[designName,nodes,edges,declaredVariables,declaredLocals]);const loadDesign=()=>{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return alert('No saved design found.');const p=JSON.parse(raw);setNodes(recalcHierarchy(p.nodes||[]));setEdges(p.edges||[]);setDesignName(p.designName||'Loaded Architecture');setDeclaredVariables(p.variables||[]);setDeclaredLocals(p.locals||[]);setSaveState('saved');setTimeout(()=>fitView({padding:.18}),0);};const download=(blob:Blob,name:string)=>{const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;a.click();URL.revokeObjectURL(u);};const safeFileName=()=>designName.replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'').toLowerCase()||'archmindcanvas-diagram';
...
<button onClick={()=>{exportDiagramSvg();setSaveMenuOpen(false)}}><Image size={15}/><span><b>Save as SVG</b><small>Scalable vector diagram</small></span></button>
 ...
 {projectVariablesOpen&&<div className="project-variables-overlay" onMouseDown={()=>setProjectVariablesOpen(false)}>
  <div className="project-variables-dialog" onMouseDown={e=>e.stopPropagation()}>
    <div className="project-variables-dialog-head">
      <div><strong>Project Variables & Locals</strong><small>{designName} · Shared across the complete architecture</small></div>
      <button onClick={()=>setProjectVariablesOpen(false)}>×</button>
    </div>
    <div className="project-variables-scope-note"><b>Project scope</b><span>These declarations belong to this design and can be referenced by any resource, tag, relationship or IaC property.</span></div>
