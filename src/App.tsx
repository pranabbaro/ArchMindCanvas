...
function Designer(){
 ...
 <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onNodeClick={(_,n)=>{if(tool==='hand')return;setSelectedNodeId(n.id);setSelectedEdgeId(undefined);setRightPanel('properties');}} onEdgeClick={(_,e)=>{if(tool==='hand')return;setSelectedEdgeId(e.id);setSelectedNodeId(undefined);setRightPanel('properties');}} onPaneClick={onPaneClick} fitView snapToGrid snapGrid={[16,16]} selectionOnDrag={tool==='select'} elementsSelectable={tool!=='hand'} nodesDraggable={tool!=='hand'} nodesConnectable={tool!=='hand'} panOnDrag={tool==='hand'} multiSelectionKeyCode="Shift" deleteKeyCode={null} defaultEdgeOptions={{type:'styled',markerEnd:{type:MarkerType.ArrowClosed},data:{connectorStyle}}}><Background variant={BackgroundVariant.Dots} gap={20} size={1.2}/><Controls position="bottom-left"/><MiniMap pannable zoomable position="bottom-right"/><div className="canvas-action"><button onClick={()=>fitView({padding:.12})}><Maximize2 size={15}/> Fit</button><button onClick={copySelection}><Copy size={15}/> Copy</button><button onClick={pasteSelection}><Clipboard size={15}/> Paste</button></div></ReactFlow></div>
