...
export default function ArchitectureNodeComponent({ data, selected }: NodeProps<ArchitectureNode>) {
  ...
  return <>
    ...
    <div className={`architecture-node ${selected?'selected':''}`}>
      ...
      <div className="node-content"><div className="node-category">Microsoft Azure · {item.category} <span className={`resource-mode-badge ${data.resourceMode||'create'}`}>{(data.resourceMode||'create').toUpperCase()}</span></div><strong>{data.label}</strong><span>{data.resourceGroup?`${data.resourceGroup} · `:''}{data.region} · {data.environment}</span></div>
