...
export default function StyledEdge(props:EdgeProps<CanvasEdge>){
 ...
 return <><BaseEdge id={`${id}-underlay`} path={path} style={{stroke:'#ffffff',strokeWidth:(data?.strokeWidth||1.8)+5,strokeLinecap:'round',strokeLinejoin:'round'}}/>
  <BaseEdge id={id} path={path} markerEnd={markerEnd} markerStart={markerStart} style={{stroke:selected?'#2563eb':'#64748b',strokeWidth:data?.strokeWidth|| (selected?3:1.8),strokeDasharray:dash}}/>{selected&&<path d={path} fill="none" stroke="transparent" strokeWidth={28} style={{pointerEvents:'stroke',cursor:'crosshair'}} onDoubleClick={addPoint}/>}<EdgeLabelRenderer>{label&&<div className={`edge-label nodrag nopan ${selected?'edge-label-movable':''}`} onPointerDown={selected?dragLabel:undefined} style={{transform:`translate(-50%, -50%) translate(${labelX}px,${labelY}px)`}}>{label}</div>}{selected&&route.map((p,i)=><div key={i} className="edge-route-handle nodrag nopan" title="Drag waypoint. Double-click to remove." onPointerDown={e=>dragPoint(i,e)} onDoubleClick={e=>removePoint(i,e)} style={{transform:`translate(-50%, -50%) translate(${p.x}px,${p.y}px)`}}/>)}</EdgeLabelRenderer></>;
}
